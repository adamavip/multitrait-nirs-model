...
def load_data(fname=None, target=None, sensor=None):
    ...
    return x_train, y_train, x_test, y_test, xscaler, yscaler, train_set_metadata, test_set_metadata



def load_data_train_test(train_fname=None, test_fname=None, target=None, sensor=None, crop=None):
    ...
    xfn = 'xscaler_'+ '_'.join([crop, target, sensor, time]) + '.gz'
    xfn = os.path.join(save_dir, xfn)
    yfn = 'yscaler_'+ '_'.join([crop, target, sensor, time]) + '.gz'
    yfn = os.path.join(save_dir, yfn)


    joblib.dump(xscaler, open(xfn, 'wb'))
    joblib.dump(yscaler, open(yfn, 'wb'))
    ...
    return x_train, y_train, x_test, xscaler, y_test,yscaler, train_set_metadata, test_set_metadata
